package com.enation.app.shop.core.table.service.impl;
...
//		}
		...
		this.daoSupport.insert("es_table", table);
		int tableid = this.daoSupport.getLastId("es_table");
	...
	public Table get(Integer tableId) {
		String sql = "select * from "
				+ "es_table where table_id=? ";
		Table t = this.daoSupport.queryForObject(sql, Table.class, tableId);
		return t;
	}	
	 ...
	 */
	@Override
	@Log(type=LogType.TABLE,detail="修改了桌位名称为${table.name}的桌位信息")
	public Table edit(Table table) {
		// 前后台用到的是一个edit方法，请在action处理好
		this.daoSupport.update("es_table", table, "table_id=" + table.getTable_id());
...
//		}
	 ...
	 */
	@Override
	@Log(type=LogType.MEMBER,detail="删除桌位")
	public void delete(Integer[] id) {
		...
		String sql = "delete from es_table where table_id in (" + str + ")";
...
//	}
	 ...
	 */
	@Override
	public Page searchTable(Map memberMap, Integer page, Integer pageSize,String other,String order) {
...
//			sql += " and m.name like '%" + uname + "%'";
	 ...
	 */
	@Override
	public Table getTableByTableId(Integer table_id) {
		String sql="select * from es_table where table_id=?";
		Table table=this.daoSupport.queryForObject(sql, Table.class, table_id);
		return table;
	}
