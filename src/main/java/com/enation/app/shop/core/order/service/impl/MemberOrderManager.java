...
public class MemberOrderManager implements IMemberOrderManager {
	...
	public Page pageOrders(int pageNo, int pageSize,String status, String keyword){
		...
		String sql = null;
		if(member.getLv_id()!=6){
			sql = "select * from es_order where member_id = '" + memberid + "' and disabled=0";
		}else{
			sql = "select * from es_order where disabled=0";
		}
		if(!StringUtil.isEmpty(status)){
//			int statusNumber = -999;
			int statusNumber = StringUtil.toInt(status);
			//等待付款的订单 按付款状态查询
//			if(statusNumber==0){
...
//			}
		...
		if(!StringUtil.isEmpty(keyword)){
			if(member.getLv_id()!=6){
				sql += " AND order_id in (SELECT i.order_id FROM es_order_items i LEFT JOIN es_order o ON i.order_id=o.order_id WHERE o.member_id='"+memberid+"' AND i.name like '%" + keyword + "%')";
			}else{
				switch (keyword) {
					case "free":
						sql += " AND paymoney=0 and status in ("+OrderStatus.ORDER_PAY+","+OrderStatus.ORDER_COMPLETE+")";
						break;
					case "gift":
						sql += " AND order_id in (SELECT i.order_id FROM es_order_items i LEFT JOIN es_order o ON i.order_id=o.order_id WHERE i.price=0)";
						break;
					case "nopay":
						sql += " AND pay_status="+OrderStatus.PAY_NO+" and status in ("+OrderStatus.ORDER_NOT_PAY+","+OrderStatus.ORDER_CONFIRM+")";
						break;
					default:
						sql += " AND order_id in (SELECT i.order_id FROM es_order_items i LEFT JOIN es_order o ON i.order_id=o.order_id WHERE i.name like '%" + keyword + "%')";
						break;
				}
