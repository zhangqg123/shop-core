 ...
 * @version V2.0
 */
@Service("tablePluginBundle")
public class TablePluginBundle extends AutoRegisterPluginsBundle {
...
//	}		
	 ...
	 * 激发会员恢复事件
	 */
	public  void onRecycle(Table table){
		try{
			List<IPlugin> plugins = this.getPlugins();
			
			if (plugins != null) {
				for (IPlugin plugin : plugins) {
					if (plugin instanceof ITableRecycleEvent) {
						if (loger.isDebugEnabled()) {
							loger.debug("调用插件 : " +plugin.getClass() + " onAddressAdd 开始...");
						}
						ITableRecycleEvent event = (ITableRecycleEvent) plugin;
						event.recycleTable(table);
						if (loger.isDebugEnabled()) {
							loger.debug("调用插件 : " +plugin.getClass() + " onAddressAdd 结束.");
						}
					} 
	...
	public  void onDeleteTable(Table table){
		try{
			...
			if (plugins != null) {
				for (IPlugin plugin : plugins) {
					if (plugin instanceof ITableDeleteEvent) {
						if (loger.isDebugEnabled()) {
							loger.debug("调用插件 : " +plugin.getClass() + " onAddressAdd 开始...");
						}
						ITableDeleteEvent event = (ITableDeleteEvent) plugin;
						event.delteTable(table);
						if (loger.isDebugEnabled()) {
							loger.debug("调用插件 : " +plugin.getClass() + " onAddressAdd 结束.");
						}
					} 
