...
public class OrderFlowManager implements IOrderFlowManager {
	...
	public Order add(Order order,List<CartItem> itemList, String sessionid) {
		...
		// 非匿名购买
		if (tmpMemberid != null) {
			member=memberManager.get(Integer.valueOf(tmpMemberid));
			order.setMember_id(Integer.valueOf(tmpMemberid));
			opname = member.getUname();
		}else{
			order.setMember_id(member.getMember_id());
			opname = member.getUname();			
		}
		order.setPeonum(member.getPeonum());
...
//		order.setPay_status(OrderStatus.PAY_NO);
		...
		for(CartItem item : itemList){
			...
			if(item.getIs_print()==3){
				isPrint=item.getIs_print();
			}
		}
//		if((order.getNeed_pay_money()==0&&order.getShip_status()!=OrderStatus.SHIP_ROG)
//			||(isPrint==3&&(order.getShip_status()==null||order.getShip_status()!=OrderStatus.SHIP_ROG))
//			||(order.getStatus()!=null&&order.getStatus()==OrderStatus.ORDER_NOT_PAY&&order.getPay_status()!=null&&order.getPay_status()==OrderStatus.PAY_NO&&order.getShip_status()!=OrderStatus.SHIP_ROG)){
//			order.setShip_status(OrderStatus.SHIP_NO);			
//		}else{
//			order.setShip_status(OrderStatus.SHIP_ROG);
//		}
		if(!result){
			throw new RuntimeException("创建订单失败，您购买的商品库存不足");
		}
		if(isPrint==OrderStatus.PAY_GIFT){
			order.setPay_status(OrderStatus.PAY_GIFT);
//			order.setPayType(3);
		}
		/**检测商品库存  End**/
...
//			throw new RuntimeException("创建订单失败，购物车为空");
		...
		//下单则自动改为已确认
		if(!order.getIsCod()){
			if(order.getStatus()==OrderStatus.ORDER_PAY
				&&(order.getPay_status()==OrderStatus.PAY_YES
				||order.getPay_status()==OrderStatus.PAY_GIFT
				||order.getPay_status()==OrderStatus.PAY_FREE
				)){
...
//			cartManager.clean(sessionid);
			cartManager.clean(member.getMember_id().toString());
			this.memberManager.cleanNumPrice(order.getMember_id());
		}
	...
	public boolean pay(Integer paymentId,Integer orderId,double payMoney,String userName) {
		...
		double paidmoney=this.orderReportManager.getPayMoney(paymentId);//其它金额中已结算的金额
			...
			if(order.getPay_status()!=OrderStatus.PAY_FREE&&CurrencyUtil.add(payMoney, paidmoney)<payment.getMoney()){
				//修改订单状态为部分付款
				...
				//更新订单的已付金额
				this.daoSupport.execute("update es_order set paymoney=paymoney+?,status=?,pay_status=? where order_id=?",payMoney,OrderStatus.ORDER_PAY,order.getPay_status(),order.getOrder_id());
	...
	public void confirmOrder(Integer orderId) {
		...
		if(order.getStatus()==OrderStatus.ORDER_NOT_PAY){
			this.daoSupport.execute("update es_order set status=? where order_id=?", OrderStatus.ORDER_CONFIRM ,orderId);			
		}else{
			this.daoSupport.execute("update es_order set status=? where order_id=?", order.getStatus() ,orderId);
			//添加一条应收记录 20131110新增
		}
		if(orderReportManager.getPaymentLogId(orderId)==0){
		...
		if(order.getStatus()==OrderStatus.ORDER_PAY
			...
			if(order.getPay_status()==OrderStatus.PAY_FREE){
				this.pay(payment_id, orderId, 0,member.getUname());				
			}else{
				this.pay(payment_id, orderId, order.getNeedPayMoney(),member.getUname());
			}
...
//		int payStatus = OrderStatus.PAY_YES;// 已付款
		int payStatus=order.getPay_status();
	...
	public void rogConfirm(int orderId,Integer op_id,String op_name,String sign_name,Long sign_time){
		Order order = this.orderManager.get(orderId);
		if(order.getStatus()==OrderStatus.ORDER_NOT_PAY&&order.getShip_status()==OrderStatus.PAY_NO){
			order.setStatus(OrderStatus.ORDER_CONFIRM);
			String sql ="update es_order set status=" + OrderStatus.ORDER_CONFIRM + ",ship_status=" + OrderStatus.SHIP_ROG + " where order_id=" + orderId;
			this.daoSupport.execute(sql);
		}else{
			if(order.getStatus()==OrderStatus.ORDER_CONFIRM&&order.getShip_status()==OrderStatus.SHIP_ROG){
				order.setPay_status(OrderStatus.PAY_YES);
				order.setStatus(OrderStatus.ORDER_PAY);
				String sql ="update es_order set status=" + order.getStatus() + ",pay_status=" + order.getPay_status() + " where order_id=" + orderId;
				this.daoSupport.execute(sql);
				this.confirmOrder(orderId);		
			}else{
