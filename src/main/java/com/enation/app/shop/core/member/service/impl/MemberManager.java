...
//			sql += " and m.name like '%" + uname + "%'";
	...
	@Override
	public void updateNum(Integer memberid, Integer peonum) {
		String sql = "update es_member set peonum=? where member_id=?";
		this.daoSupport.execute(sql, peonum, memberid);
	}

	@Override
	public void cleanNumPrice(Integer memberid) {
		String sql = "update es_member set peonum=?, goods_price=? where member_id=?";
		this.daoSupport.execute(sql, 0, 0, memberid);
	}
