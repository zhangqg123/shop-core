...
public class OrderController extends GridController {
	...
	public ModelAndView list() {
		...
		for (int i = 0; i < statusList.size(); i++) {
			Map x=(Map) statusList.get(i);
			String key = (String) x.get("key");
		    if(x.get("key").equals(""+OrderStatus.ORDER_SHIP)
		    	||x.get("key").equals(""+OrderStatus.ORDER_ROG)
		    	||x.get("key").equals(""+OrderStatus.ORDER_MAINTENANCE)){
...
//			return JsonResultUtil.getSuccessJson("确认收货成功");
			return JsonResultUtil.getSuccessJson("确认订单完成");
		} catch (Exception e) {
