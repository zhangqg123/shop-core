...
public class CartManager implements ICartManager {
	...
	public int add(Cart cart) {
		...
		if(tmpMemberid==null){
			...
			cart.setMember_id(Integer.valueOf(tmpMemberid));
			//管理员下单
			cart.setIs_print(2);
		}
		/**
...
//			sql ="select count(0) from es_cart where  product_id=?   and itemtype=? and ( session_id=? or member_id=? )";
//		}
		/**
...
//		int count=0;
//		count = this.daoSupport.queryForInt(sql, cart.getProduct_id(),cart.getItemtype(),cart.getSession_id(),memberid);
...
//		}
	...
	public List<CartItem> listGoods(String sessionid) {
		...
		sql.append(" c.is_check,c.is_print,c.cart_id,g.cat_id as catid,g.goods_id,g.thumbnail,c.name ,  p.sn, p.specs  ,g.mktprice,g.unit,g.point,p.product_id,c.price,c.cart_id as cart_id,c.num as num,c.itemtype,c.addon,c.weight,c.activity_id from es_cart c, es_product p,es_goods g ");
		...
		if(tmpMemberid==null){
			...
			sql.append(" and c.member_id=? and c.is_print>?");
			list = this.daoSupport.queryForList(sql.toString(), new CartItemMapper(), tmpMemberid, 0);			
		}
	...
	@Override
	public List<CartItem> tableListGoods(String memberid) {
		//更新需要同步的商品
//		this.reReorganization();
...
//		}else{
			sql.append(" and c.member_id=?");
			list = this.daoSupport.queryForList(sql.toString(), new CartItemMapper(), memberid);
//		}
...
//		cartPluginBundle.filterList(list, sessionid);
	...
	public List<CartItem> selectListGoods(String sessionid) {
		...
		sql.append(" c.is_check,c.is_print,g.cat_id as catid,g.goods_id,g.thumbnail,c.name ,  p.sn, p.specs  ,g.mktprice,g.unit,g.point,p.product_id,c.price,c.cart_id as cart_id,c.num as num,c.itemtype,c.addon,c.weight,c.activity_id  from es_cart c, es_product p,es_goods g ");
		...
		if(tmpMemberid==null){
			...
			sql.append(" and c.member_id=?");
			list = this.daoSupport.queryForList(sql.toString(), new CartItemMapper(), tmpMemberid);			
		}
	...
	public void clean(String sessionid){
		...
		if(tmpMemberid==null){
			...
			String sql ="delete from es_cart where is_check=1 and (session_id=? or member_id=?)  ";
			this.daoSupport.execute(sql, sessionid,tmpMemberid);			
		}
	}
	...
	public void delete(String sessionid, Integer cartid) {
		...
		if(tmpMemberid==null){
			...
			sql = "delete from es_cart where member_id = " + tmpMemberid + " and cart_id = ?";
		}
...
//		HttpCacheManager.sessionChange();
	...
	public void updateNum(String sessionid, Integer cartid, Integer num) {
		try {
			...
			if(tmpMemberid==null){			
				...
				sql = "update es_cart set num = ? where member_id = " + tmpMemberid + " and cart_id = ?";
			}
	...
	@Override
	public void updatePrice(String sessionid, Integer cartid) {
		try {		
			//is_print =3 是赠品
			String sql = "";
			String tmpMemberid=(String) ThreadContextHolder.getSession().getAttribute(UserConext.CURRENT_TABLE_KEY);
			if(tmpMemberid==null){			
				...
				sql = "update es_cart set is_print = ? where member_id = " + tmpMemberid + " and cart_id = ?";
				this.daoSupport.execute(sql, 3, cartid);
			}
		}catch (RuntimeException e) {
			throw e;
		}
	...
	public OrderPrice countPrice(List<CartItem> cartItemList, Integer shippingid,String regionid) {
		...
		if(cartItemList!=null && !cartItemList.isEmpty()){
			...
			for (CartItem cartItem : cartItemList) {
				...
				if(cartItem.getIs_print()!=3){
					needPayMoney=CurrencyUtil.add(needPayMoney, itemTotal);
				}
		...
		orderPrice.setNeedPayMoney(CurrencyUtil.add(needPayMoney,-orderPrice.getDiscountPrice())); 
...
//	public void checkProduct(String sessionid,Integer product_id, boolean check) {
	public void checkProduct(String sessionid,Integer cart_id, boolean check) {
		
		Member member  =UserConext.getCurrentMember();
		
//		StringBuffer sql=new StringBuffer("update es_cart set is_check=? where product_id=?");
		StringBuffer sql=new StringBuffer("update es_cart set is_check=? ,is_print=? where cart_id=?");
		...
		if(tmpMemberid==null){
			if(member==null){
				...
				this.daoSupport.execute(sql.toString(), check==true?1:0, 0,cart_id, sessionid);
			}else{
				sql.append(" and member_id=?");
				this.daoSupport.execute(sql.toString(), check==true?1:0, 0,cart_id, member.getMember_id());
			}
		}else{
			sql.append(" and member_id=?");
			this.daoSupport.execute(sql.toString(), check==true?1:0,check==true?1:0,cart_id, tmpMemberid);			
		}
	...
	public void placeOrder(String sessionid) {
		Member member  =UserConext.getCurrentMember();
		StringBuffer sql=new StringBuffer("update es_cart set is_print=? ");
				
		if(member==null){
			sql.append(" where session_id=? and is_check =1 and (is_print is null or is_print!=1);");
			this.daoSupport.execute(sql.toString(), 1,sessionid);
		}else{
			sql.append(" where member_id=? and is_check =1 and (is_print is null or is_print!=1);" );
			this.daoSupport.execute(sql.toString(), 1, member.getMember_id());
		}
//		StringBuffer sql2=new StringBuffer("update es_member set goods_price=? ");
//		
//		if(member!=null){
//			sql2.append(" where member_id=?;" );
//			this.daoSupport.execute(sql2.toString(), goodsprice, member.getMember_id());
//			result=1;
//		}
	}
