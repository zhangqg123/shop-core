...
//				}
				...
				int cartid = this.cartManager.add(cart);
				Cart newCart = this.cartManager.get(cartid);
				if(newCart.getIs_print()==2){
					handler.sendMessageToUser(4L, new TextMessage("adminAdd"));;
				}
...
//	public JsonResult addCart(Product product, int tableid, int num, int showCartData, Integer activity_id){
//
//		
//		String sessionid =ThreadContextHolder.getHttpRequest().getSession().getId();
//		if(product!=null){
...
//				//查询已经存在购物车里的商品
//				
//				Cart tempCart = cartManager.getCartByProductId(product.getProduct_id(), tableid);
//				if(tempCart != null){
//					int tempNum = tempCart.getNum();
//					if (enableStore < num + tempNum) {
//						throw new RuntimeException("抱歉！您所选择的货品库存不足。");
